...
# rule process_novel_and_rare_variants:
    # input:
        # expand("results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.genic.deleterious.counts.txt", VAR_TYPE = var_type, SAMPLEID = probandid),
        # expand("results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.genic.deleterious.counts.txt", VAR_TYPE = var_type, SAMPLEID = probandid)

# rule process_rare_variants:
    # input:
        # expand("results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.genic.deleterious.txt", VAR_TYPE = var_type, SAMPLEID = probandid)

# rule rare_summarize_filtering_steps:
    # input:
        # multianno_file = "results/annovar_output/multiannotation/indiv_samples/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.hg19_multianno.txt",
        # rare_file = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.txt",
        # noncoding_file = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.noncoding.txt",
        # exonic_file = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.txt",
        # nonsynonymous_file = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.nonsyn.txt",
        # deleterious_file = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.deleterious.txt"
    # output:
        # "results/annovar_output/variant_filtering/remaining_22q11/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.remaining22q11.filtering_summary.txt"
    # shell: """
    # echo -e "Filtering step\tVariants" > {output}
    # grep -v -P "Chr\tStart" {input.multianno_file} | grep -v -E "^#" | awk 'END {{print "22q11.2 deleted region\t", NR}}' >> {output}
    # # total variants
    # grep -v -P "Chr\tStart" {input.rare_file} | grep -v -E "^#" | awk 'END {{print "22q11.2 deleted region\t", NR}}' >> {output}
    # # non-coding variants
    # grep -v -P "Chr\tStart" {input.noncoding_file} | grep -v -E "^#" | awk 'END {{print "Non-coding\t", NR}}' >> {output}
    # # Exonic variants
    # grep -v -P "Chr\tStart" {input.exonic_file} | grep -v -E "^#" | awk 'END {{print "Exonic\t", NR}}' >> {output}
    # # Non-synonymous variants
    # grep -v -P "Chr\tStart" {input.nonsynonymous_file} | grep -v -E "^#" | awk 'END {{print "Non-synonymous\t", NR}}' >> {output}
    # # Deleterious variants
    # grep -v -P "Chr\tStart" {input.deleterious_file} | grep -v -E "^#" | awk 'END {{print "Deleterious\t", NR}}' >> {output}
    # """

# rule rare_summarize_variant_filtering:
    # input: 
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.genic.deleterious.txt"
    # output: 
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.genic.deleterious.counts.txt"
    # shell: """
...
# rule rare_extract_deleterious:
    # input:
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.nonsyn.txt"
    # params: deleterious_score_cutoff = "2"
    # output:
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.deleterious.txt"
    # run:
        # extract_deleterious_variants(input[0], output[0], params.deleterious_score_cutoff)

# rule rare_extract_nonsynonymous:
    # input:
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.txt"
    # output:
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.nonsyn.txt"
    # message:"""
...
# rule rare_parse_coding_noncoding:
    # input: "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.txt"
    # output:
        # exonic = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.exonic.txt",
        # noncoding = "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.noncoding.txt"
    # run:
        # extract_exonic_variants(input[0], output[0])

# rule rare_variants:
    # input:
        # "results/annovar_output/variant_filtering/all_variants/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.txt"
    # params: maf_cutoff = "0.05"
    # output:
        # "results/annovar_output/variant_filtering/rare/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.rare.txt"
    # run:
        # extract_rare_variants(input[0], output[0], params.maf_cutoff)
...
# rule process_novel_variants:
    # input:
        # expand("results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.exonic.deleterious.novel.counts.txt", VAR_TYPE = var_type, SAMPLEID = probandid)

# rule novel_summarize_filtering_steps:
    # input:
        # multianno_file = "results/annovar_output/multiannotation/indiv_samples/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.hg19_multianno.txt",
        # novel_file = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.txt",
        # noncoding_file = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.noncoding.txt",
        # exonic_file = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.txt",
        # nonsynonymous_file = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.nonsyn.txt",
        # deleterious_file = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.deleterious.txt"
    # output:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.novel.filtering_summary.txt"
    # shell: """
    # echo -e "Filtering step\tVariants" > {output}
    # grep -v -P "Chr\tStart" {input.multianno_file} | grep -v -E "^#" | awk 'END {{print "22q11.2 deleted region\t", NR}}' >> {output}
    # # total variants
    # grep -v -P "Chr\tStart" {input.novel_file} | grep -v -E "^#" | awk 'END {{print "22q11.2 deleted region\t", NR}}' >> {output}
    # # non-coding variants
    # grep -v -P "Chr\tStart" {input.noncoding_file} | grep -v -E "^#" | awk 'END {{print "Non-coding\t", NR}}' >> {output}
    # # Exonic variants
    # grep -v -P "Chr\tStart" {input.exonic_file} | grep -v -E "^#" | awk 'END {{print "Exonic\t", NR}}' >> {output}
...
# rule novel_summarize_variant_filtering:
    # input: 
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.exonic.deleterious.novel.txt"
    # output: 
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.exonic.deleterious.novel.counts.txt"
    # shell: """
        # grep -v -P "Chr\tStart" {input} | grep -v -E "^#" | awk 'END {{print "total\t", NR}}' > {output}
        # grep -w "exonic" {input} | awk 'END {{print "Exonic\t", NR}}' >> {output}
        # grep -w "nonsynonymous" {input} | awk 'END {{print "Nonsynonymous\t", NR}}' >> {output}
...
# rule novel_extract_deleterious_indiv:
    # input:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.nonsyn.txt"
    # params: deleterious_count_cutoff = "2"
    # output:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.deleterious.txt"
    # message: """
...
# rule novel_extract_nonsynonymous_indiv:
    # input:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.txt"
    # output:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.nonsyn.txt"
    # message: """
    # Extract non-synonymous variants.
    ...
    # """
    # run:
        # extract_nonsynonymous_variants(input[0], output[0], params.deleterious_count_cutoff)

# rule novel_parse_coding_noncoding_indiv:
    # input:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.txt"
    # output:
        # exonic = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.exonic.txt",
        # noncoding = "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.noncoding.txt"
    # message:"""
...
# rule novel_indiv:
    ...
    # output:
        # "results/annovar_output/variant_filtering/novel/wgs_phased_{VAR_TYPE}.sample.{SAMPLEID}.nosuperdup.novel.txt"
    # message:"""
    # Removing variants that were previously reported in dbsnp.
    
    # dbSNP version: {params.dbsnp_version}
    # Input: {input}
    # Ouptut: {output}
    # """
    # run:
        # extract_novel_variants(input[0], output[0], params.dbsnp_version, params.esp_database, params.thousand_genomes_database)
